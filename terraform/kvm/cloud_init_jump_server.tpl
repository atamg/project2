...
runcmd:
  ...
  - 'blkid "$VAR_DEV" || mkfs.ext4 -L var "$VAR_DEV"'
  - 'blkid "$HOME_DEV" || mkfs.ext4 -L home "$HOME_DEV"'
  ...
  - 'echo "UUID=$(blkid -s UUID -o value /dev/disk/by-label/var)  /var  ext4   defaults        0 2"  >> /etc/fstab'
  - 'echo "UUID=$(blkid -s UUID -o value /dev/disk/by-label/home) /home ext4   defaults,nodev 0 2"  >> /etc/fstab'
